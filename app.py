...
with tabs[1]:
    ...
    with col_idx:
        st.metric("加權指數", f"{S_current:,.0f}", delta=f"{S_current-ma20:.0f} (vs MA20)")
    
    with col_trend:
        if trend_score == 2:
            st.metric("目前趨勢", "🔥 強勢多頭", "適合 Buy CALL")
            color = "green"
        elif trend_score == 1:
            st.metric("目前趨勢", "⚖️ 震盪整理", "觀望 / 賣方")
            color = "orange"
        else:
            st.metric("目前趨勢", "❄️ 空頭修正", "現金為王 / Buy PUT")
            color = "red"
            
    with col_action:
        st.markdown(f"### 建議動作")
        if color == "green":
            st.success("✅ **綠燈：前往「CALL獵人」尋找機會**")
        elif color == "orange":
...
with tabs[3]:
    ...
    with col_p2:
        st.markdown("#### 📉 **損益試算**")
        k_sim = st.number_input("模擬履約價", 15000, 30000, int(S_current))
