"""
🔰 台指期權雙模式系統
- 模式1：簡易新手機 (單純、快速、10大警示)
- 模式2：終極戰情室 (投組管理、風險監控、自由篩選)
- 一鍵切換，滿足不同需求
"""
...
app_mode = st.sidebar.radio("選擇模式", ["🔰 簡易新手機 (新手推薦)", "🔥 終極戰情室 (投組管理)"])
...
# 模式 1：簡易新手機 (你原本的介面)
# ==========================================
if app_mode == "🔰 簡易新手機 (新手推薦)":
    st.markdown("# 🔥 **台指期權新手機** (專業術語版)")
    
    # 新手教學 (折疊)
    with st.expander("📚 **新手村：3分鐘看懂你在選什麼**", expanded=False):
        st.markdown("""
        ### 🐣 **Call vs Put**
        *   **CALL (買權)** 📈：看漲
        *   **PUT (賣權)** 📉：看跌
...
# 模式 2：終極戰情室 (含投組管理)
# ==========================================
elif app_mode == "🔥 終極戰情室 (投組管理)":
    st.markdown("# 🔥 **終極戰情室** (投組管理版)")
    ...
    with col_search:
        ...
        with c1:
            dir_mode = st.selectbox("方向", ["CALL 📈", "PUT 📉"])
        ...
        with c2:
            if not df_latest.empty:
                ...
                sel_con_2 = st.selectbox("合約", future_c, index=len(future_c)-1 if future_c else 0)
            else: sel_con_2 = ""
        with c3:
            lev_2 = st.slider("槓桿", 2.0, 15.0, 5.0)
        ...
        if 'best_match' in st.session_state and st.session_state.best_match:
            ...
            if st.button("➕ 加入投組", key="add_pf"):
                ...
                if not exists: st.session_state.portfolio.append(b)
