...
if st.button("🎯 **全開計算！**", type="primary", use_container_width=True):
    ...
    st.info(f"2️⃣ 📊 **雙面情境**：台指若做對 100 點可賺 **${profit_100:,}**；但若做錯 100 點將虧損 **同樣金額**，最壞情況權利金歸零。")

    # 3. 資金控管 (強制性)
    contract_cost = best['參考價'] * 50
    st.error(f"3️⃣ 💰 **資金鐵律**：1 口成本 **${int(contract_cost):,}**。請確保這是閒錢，且本金至少有 **20倍** 緩衝，否則請勿操作！")

    # 4. 勝率解析
    wr = best['勝率']
    st.markdown(f"4️⃣ 📉 **勝率解讀**：歷史統計勝率約 **{wr}%**，但這代表仍有 **{100-wr:.0f}%** 的機率會賠錢，請做好心理準備。")

    # 5. Delta 性格
    delta = best['Delta']
    if delta > 0.8:
        st.markdown(f"5️⃣ 🧠 **波動特性**：Delta {delta} (高)，跟漲跟跌都強烈，心臟要大顆。")
    elif delta > 0.5:
        st.markdown(f"5️⃣ 🧠 **波動特性**：Delta {delta} (中)，有感度但風險相對可控。")
    else:
        st.markdown(f"5️⃣ 🧠 **波動特性**：Delta {delta} (低)，平常不太動，容易因時間價值流失而歸零。")

    # 6. 停損鐵律
    st.error("6️⃣ 🛑 **停損鐵律**：權利金虧損達 **20%** 時，請立即平倉！留得青山在，不怕沒柴燒。")
    
    # 7. 倉位限制
    st.warning("7️⃣ ⚖️ **倉位限制**：總帳戶請勿超過 **10%** 資金買期權，單一合約請勿超過 **2%** 本金。")

    # 8. 時間風險
    if days_left > 30:
        st.info("8️⃣ ⏰ **時間風險**：屬遠月合約，時間價值流失較慢，但需耐心等待。")
    elif days_left > 7:
        st.warning("8️⃣ ⏰ **時間風險**：屬近月合約，時間價值流失快，適合短線操作。")
    else:
        st.error("8️⃣ ⏰ **時間風險**：即將到期！時間價值每日大減，歸零風險極高，強烈建議平倉。")
        
    # 9. 心態提醒
    st.markdown("9️⃣ 🧘 **心態提醒**：期權不是賭博！請設定好停損停利點，**絕不凹單**。")
    
    # 10. 最終警示
    st.error("🔟 🚨 **最終警示**：期權具有 **100% 歸零風險**，請務必使用閒錢操作，切勿借錢投資！")
